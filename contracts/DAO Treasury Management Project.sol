...
contract GovernanceTimelock is AccessControl, ReentrancyGuard, Pausable {
    ...
    function getProposal(uint256 _id) external view returns (
        ...
        return (
            p.id, p.proposer, p.target, p.value,
            p.description, p.createdAt, p.approvedAt,
            p.state, p.yesVotes, p.noVotes
        );
    }
    ...
    // 🔥 New 4 Smart Functions

    function getProposalStateReason(uint256 _proposalId) external view returns (string memory) {
        Proposal storage p = proposals[_proposalId];
        if (p.state == ProposalState.Pending) return "Proposal is pending vote or approval";
        if (p.state == ProposalState.Approved) return "Proposal approved and waiting for execution";
        if (p.state == ProposalState.Rejected) return "Proposal rejected by admin";
        if (p.state == ProposalState.Canceled) return "Proposal canceled by proposer";
        if (p.state == ProposalState.Executed) return "Proposal executed successfully";
        if (p.state == ProposalState.Expired) return "Proposal expired";
        return "Unknown state";
    }

    function hasProposalExpired(uint256 _proposalId) public view returns (bool) {
        Proposal storage p = proposals[_proposalId];
        return block.timestamp > p.createdAt + proposalExpiry;
    }

    function countVotesForProposal(uint256 _proposalId) external view returns (uint256 totalVotes) {
        Proposal storage p = proposals[_proposalId];
    ...
    function getProposalsByState(ProposalState _state) external view returns (uint256[] memory) {
        uint256 count;
        for (uint256 i = 0; i < allProposalIds.length; i++) {
            if (proposals[allProposalIds[i]].state == _state) {
                count++;
            }
        }
